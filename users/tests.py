...
class UsersTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create(
            email="whoare@mail.com"
        )  # Создаем обычного пользователя

        self.user = User.objects.create(
            email="test@test.com",  # Создаем суперпользователя
            password="1234",
            is_staff=True,
            is_superuser=True,
        )
    ...
    def test_create_user(self):
        ...
        self.assertEqual(
            response.json()["email"], data["email"]
        )  # Проверяем, что email в ответе совпадает с отправленным
    ...
    def test_update_user(self):
        ...
        user_update = (
            User.objects.create(  # Создаем пользователя, которого будем обновлять
        ...
        self.assertEqual(
            response.json()["email"], "ktoto@kto.ru"
        )  # Проверяем, что email
